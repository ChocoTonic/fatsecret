...
def _index_simple_types(root: ET.Element) -> dict[str, ET.Element]:
    return {
        s.get("name"): s
        for s in root.findall(XSD_NS + "simpleType")
        if s.get("name")
    }
...
def _index_elements(root: ET.Element) -> dict[str, ET.Element]:
    return {
        e.get("name"): e
        for e in root.findall(XSD_NS + "element")
        if e.get("name")
    }
...
class _FieldSpec:
    ...
    __slots__ = ("name", "annotation", "optional", "is_list", "imports", "common_aliases")
...
def _resolve_field_type(
    ...
    if type_name is None:
        ...
        log.warning("element %s/%s has no type and no inline complex", parent_name, field_name)
    ...
    if type_name in complex_types:
        ...
        if ctype.find(XSD_NS + "sequence") is None:
            ...
            if restriction is not None and restriction.get("base") in _XSD_PRIMITIVE_MAP:
                return _XSD_PRIMITIVE_MAP[restriction.get("base")], imports, aliases, deps
        cls = _snake_to_pascal(type_name)
        deps.add(cls)
        return cls, imports, aliases, deps

    # 7) Unknown — log and fall back to str.
    log.warning("unresolved type %r at %s/%s; using str", type_name, parent_name, field_name)
    return "str", imports, aliases, deps
...
def emit_models(resource: str) -> Path:
    ...
    if builder is None:
        raise ValueError(
            ...
            f"known: {sorted(_RESOURCE_BUILDERS)}"
        )
    classes = builder()
